...
<template>
    <li class="nav-item nav-profile dropdown">
        <a class="d-flex align-items-center nav-link dropdown-toggle mr-0" id="profileDropdown" href="#"
            ...
            <avatar :title="user.full_name" avatar-class="avatars-w-40" :img="user.profile_picture?.full_url || ''"
                alt-text="image" />
        </a>
        <div class="dropdown-menu dropdown-menu-right navbar-dropdown" aria-labelledby="profileDropdown">
            <div class="dropdown-item profile">
                <avatar :title="user.full_name" avatar-class="avatars-w-50" :img="user.profile_picture?.full_url || ''"
                    alt-text="image" />
            ...
            <a class="dropdown-item" v-for="(item, index) in data" :href="item.url" :key="index">
                <app-icon class="mr-3" :type="item.optionIcon" />
                {{ item.optionName }}
            </a>
        </div>
    </li>
